// import { notFound } from 'next/navigation'

// type Materi = {
//   id: number
//   title: string
//   content?: string
// }

// // Gantilah ini dengan URL base kamu di production nanti
// const BASE_URL = process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'

// // Fungsi untuk membuat slug dari title
// function slugify(text: string): string {
//   return text
//     .toLowerCase()
//     .replace(/[^\w\s-]/g, '') // hapus simbol aneh
//     .replace(/\s+/g, '-')     // ganti spasi dengan -
//     .replace(/--+/g, '-')     // hindari double dash
//     .trim()
// }

// export default async function MateriDetailPage({ params }: { params: { id: string } }) {
//   const res = await fetch(`${BASE_URL}/api/materials/${params.id}`, {
//     cache: 'no-store',
//   })

//   if (!res.ok) return notFound()

//   const materi: Materi = await res.json()

//   return (
//     <div className="p-4 max-w-2xl mx-auto">
//       <h1 className="text-2xl font-bold text-pink-600">{materi.title}</h1>

//       {materi.title && (
//         <img
//           src={`/image/materi/${slugify(materi.title)}.png`}
//           alt={materi.title}
//           className="my-4 w-full max-w-md mx-auto rounded-lg shadow-md"
//         />
//       )}

//       <p className="text-gray-700 leading-relaxed text-justify">{materi.content}</p>
//     </div>
//   )
// }

// // Path: src/app/materi/[id]/page.tsx

// "use client";

// import { useParams } from 'next/navigation';
// import { useEffect, useRef, useState } from 'react';
// import Image from "next/image";

// // Definisikan tipe data materi
// type Materi = {
//   id: number;
//   title: string;
//   content?: string;
// };

// // Fungsi slugify, pastikan menangani input null/undefined
// function slugify(text: string | null | undefined): string {
//   if (!text) return '';
//   return text.toLowerCase().replace(/[^\w\s-]/g, '').replace(/\s+/g, '-').replace(/--+/g, '-').trim();
// }

// export default function MateriDetailPage() {
//   const params = useParams();
//   const materialId = Array.isArray(params.id) ? params.id[0] : params.id;

//   const [materi, setMateri] = useState<Materi | null>(null);
//   const [isLoading, setIsLoading] = useState(true);
//   const [error, setError] = useState<string | null>(null);
//   const bottomRef = useRef<HTMLDivElement>(null);
//   const hasCompleted = useRef(false);
//   const childId = 1; // Harusnya dinamis

//   useEffect(() => {
//     if (!materialId) {
//       setError("ID Materi tidak valid.");
//       setIsLoading(false);
//       return;
//     }

//     const fetchMateriAndStartProgress = async () => {
//       setIsLoading(true);
//       setError(null);
//       try {
//         const res = await fetch(`/api/materials/${materialId}`);
//         if (!res.ok) throw new Error("Gagal memuat detail materi.");
//         const data: Materi = await res.json();
//         setMateri(data);

//         await fetch('/api/progress/start-material', {
//           method: 'POST',
//           headers: { 'Content-Type': 'application/json' },
//           body: JSON.stringify({ childId, materialId: parseInt(materialId) }),
//         });
//       } catch (e) {
//         const errorMessage = e instanceof Error ? e.message : "Terjadi kesalahan tidak diketahui.";
//         setError(errorMessage);
//       } finally {
//         setIsLoading(false);
//       }
//     };

//     fetchMateriAndStartProgress();
//   }, [materialId]);

//   useEffect(() => {
//     if (!materi || !bottomRef.current || !materialId) return;

//     const observer = new IntersectionObserver(
//       (entries) => {
//         if (entries[0].isIntersecting && !hasCompleted.current) {
//           hasCompleted.current = true;
//           fetch('/api/progress/complete-material', {
//             method: 'POST',
//             headers: { 'Content-Type': 'application/json' },
//             body: JSON.stringify({ childId, materialId: parseInt(materialId) }),
//           })
//           .then(res => {
//             if(!res.ok) console.error("API complete-material gagal", res);
//             else console.log("API complete-material BERHASIL.");
//           })
//           .catch(error => console.error("FATAL ERROR saat fetch complete-material:", error));
//           observer.disconnect();
//         }
//       },
//       { threshold: 1.0 }
//     );
//     observer.observe(bottomRef.current);
//     return () => observer.disconnect();
//   }, [materi, materialId]);

//   if (isLoading) return <div className="p-4 text-center">Memuat materi...</div>;
//   if (error) return <div className="p-4 text-center text-red-500">Error: {error}</div>;
//   if (!materi) return <div className="p-4 text-center">Materi tidak ditemukan.</div>;

//   return (
//     <div className="p-4 max-w-2xl mx-auto">
//       <h1 className="text-2xl font-bold text-pink-600">{materi.title}</h1>
//       <Image
//         src={`/image/materi/${slugify(materi.title)}.png`}
//         alt={materi.title || 'Gambar Materi'}
//         width={700} height={400}
//         className="my-4 w-full max-w-md mx-auto rounded-lg shadow-md object-cover"
//         onError={(e) => { e.currentTarget.src = 'https://placehold.co/700x400/fecdd3/4c0519?text=Gambar+Materi'; }}
//         priority
//       />
//       <p className="text-gray-700 leading-relaxed text-justify">{materi.content}</p>
//       <div ref={bottomRef} style={{ height: '50px' }} />
//     </div>
//   );
// }

// Path: src/app/materi/[id]/page.tsx

"use client";

import { useParams } from 'next/navigation';
import { useEffect, useRef, useState } from 'react';
import Image from "next/image";

// Definisikan tipe data materi
type Materi = {
  id: number;
  title: string;
  content?: string;
};

function slugify(text: string | null | undefined): string {
  if (!text) return '';
  return text.toLowerCase().replace(/[^\w\s-]/g, '').replace(/\s+/g, '-').replace(/--+/g, '-').trim();
}

export default function MateriDetailPage() {
  const params = useParams();
  const materialId = Array.isArray(params.id) ? params.id[0] : params.id;

  const [materi, setMateri] = useState<Materi | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  
  const bottomRef = useRef<HTMLDivElement>(null);
  const hasCompleted = useRef(false);
  const timeSpentRef = useRef(0);
  const childId = 1; // Harusnya dinamis

  // useEffect untuk mengambil data materi & memulai progress
  useEffect(() => {
    if (!materialId) {
      setError("ID Materi tidak valid.");
      setIsLoading(false);
      return;
    }
    
    // --- KODE PENGAMBILAN DATA YANG DIKEMBALIKAN ---
    const fetchMateriAndStartProgress = async () => {
      setIsLoading(true);
      setError(null);
      try {
        const res = await fetch(`/api/materials/${materialId}`);
        if (!res.ok) throw new Error("Gagal memuat detail materi.");
        const data: Materi = await res.json();
        setMateri(data);

        await fetch('/api/progress/start-material', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ childId, materialId: parseInt(materialId) }),
        });
      } catch (e) {
        const errorMessage = e instanceof Error ? e.message : "Terjadi kesalahan tidak diketahui.";
        setError(errorMessage);
      } finally {
        setIsLoading(false);
      }
    };
    // ------------------------------------------------

    fetchMateriAndStartProgress();
  }, [materialId]);

  // useEffect untuk timer real-time
  useEffect(() => {
    let timer: NodeJS.Timeout;
    
    const sendTimeData = () => {
      if (timeSpentRef.current > 0) {
        navigator.sendBeacon(
          '/api/progress/update-time', 
          JSON.stringify({ seconds: timeSpentRef.current })
        );
      }
    };
    
    const handleVisibilityChange = () => {
      if (document.hidden) {
        clearInterval(timer);
      } else {
        timer = setInterval(() => { timeSpentRef.current += 1; }, 1000);
      }
    };
    
    timer = setInterval(() => { timeSpentRef.current += 1; }, 1000);
    document.addEventListener('visibilitychange', handleVisibilityChange);
    window.addEventListener('beforeunload', sendTimeData);

    return () => {
      clearInterval(timer);
      sendTimeData();
      document.removeEventListener('visibilitychange', handleVisibilityChange);
      window.removeEventListener('beforeunload', sendTimeData);
    };
  }, []);

  // useEffect untuk scroll (tidak berubah)
  useEffect(() => {
    if (!materi || !bottomRef.current || !materialId) return;

    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting && !hasCompleted.current) {
          hasCompleted.current = true;
          fetch('/api/progress/complete-material', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ childId, materialId: parseInt(materialId) }),
          })
          .then(res => {
            if(!res.ok) console.error("API complete-material gagal", res);
            else console.log("API complete-material BERHASIL.");
          })
          .catch(error => console.error("FATAL ERROR saat fetch complete-material:", error));
          observer.disconnect();
        }
      },
      { threshold: 1.0 }
    );
    observer.observe(bottomRef.current);
    return () => observer.disconnect();
  }, [materi, materialId]);


  if (isLoading) return <div className="p-4 text-center">Memuat materi...</div>;
  if (error) return <div className="p-4 text-center text-red-500">Error: {error}</div>;
  if (!materi) return <div className="p-4 text-center">Materi tidak ditemukan.</div>;

  return (
    <div className="p-4 max-w-2xl mx-auto">
      <h1 className="text-2xl font-bold text-pink-600">{materi.title}</h1>
      <Image
        src={`/image/materi/${slugify(materi.title)}.png`}
        alt={materi.title || 'Gambar Materi'}
        width={700} height={400}
        className="my-4 w-full max-w-md mx-auto rounded-lg shadow-md object-cover"
        onError={(e) => { e.currentTarget.src = 'https://placehold.co/700x400/fecdd3/4c0519?text=Gambar+Materi'; }}
        priority
      />
      <p className="text-gray-700 leading-relaxed text-justify">{materi.content}</p>
      <div ref={bottomRef} style={{ height: '50px' }} />
    </div>
  );
}
